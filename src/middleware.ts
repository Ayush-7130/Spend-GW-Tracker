...
const PROTECTED_ROUTES = [
  ...
  "/profile",
  "/security",
  "/groups", // Include groups page in protected routes
];

/**
 * Routes that don't require group validation (even when authenticated)
 * NOTE: Group requirement is now handled by individual routes using withGroupAuth
 * Middleware only validates authentication, not group membership
 */
...
const PUBLIC_ROUTES = [
  ...
  "/api/groups", // Allow group API access for creating first group
];
...
function addSecurityHeaders(response: NextResponse): NextResponse {
  ...
  // Development: Permissive for Next.js hot reload and dev tools
  // Production: Strict policy with unsafe-inline for React hydration (required by Next.js)
  //   - Removed unsafe-eval for better XSS protection
  //   - Use nonce-based approach if unsafe-inline needs to be removed later
  response.headers.set(
    "Content-Security-Policy",
    process.env.NODE_ENV === "production"
      ? "default-src 'self'; " +
          "script-src 'self' 'unsafe-inline'; " + // unsafe-inline required for Next.js React hydration
          "style-src 'self' 'unsafe-inline'; " +
          "img-src 'self' data: https:; " +
          "font-src 'self' data:; " +
          "connect-src 'self'; " +
          "frame-ancestors 'none';"
      : "default-src 'self' 'unsafe-inline' 'unsafe-eval'; " + // Dev mode: permissive for HMR
          "img-src 'self' data: https:; " +
          "font-src 'self' data:;"
  );
...
export async function middleware(request: NextRequest) {
  ...
  if (isProtectedRoute) {
    ...
    const payload = await verifyRefreshToken(token);
    if (payload) {
      // FIX C3: Removed self-fetch to /api/groups from middleware
      // Group membership validation is handled by individual routes using withGroupAuth
      // This eliminates circular dependency and improves reliability
