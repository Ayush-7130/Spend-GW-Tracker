...
export const semanticColors = {
  // User colors will be dynamically generated based on username
  // No hard-coded user-specific colors
...
/**
 * Get user color dynamically based on username hash
...
export function getUserColor(
  ...
  const hash = user.split("").reduce((acc, char) => {
    return char.charCodeAt(0) + ((acc << 5) - acc);
  }, 0);

  const lightColors = ["#A294F9", "#22C55E", "#8B5CF6", "#F59E0B", "#06B6D4"];
  const darkColors = ["#3B82F6", "#10B981", "#6D28D9", "#D97706", "#0891B2"];
...
// All colors should now use CSS variables from themes.css
// or the lightTheme/darkTheme constants above for JS usage
// ===========================================================================
