...
export default function Table<T = any>({
  ...
  const processedData = useMemo(() => {
    // ARCHITECTURE FIX: Ensure data is always an array
    // Guards against API/datasource mismatches that might return undefined or non-array
    if (!Array.isArray(config.data)) {
      logger.warn("Table received non-array data", {
  ...
  const paginatedData = useMemo(() => {
    ...
    // ARCHITECTURE FIX: Safe array length check
    ...
    if (config.pagination.total && config.pagination.total > dataLength) {
  ...
  }, [config.pagination, config.data, processedData.length]);
    ...
    (row: T, selected: boolean) => {
      ...
      if (selected) {
        ...
        newSelected = currentSelected.filter(
          (r, ri) => config.keyExtractor(r, ri) !== rowId
        );
      }
    ...
    <div className={className}>
                ...
                <div key={filter.key} className="col-md-3">
                  {filter.type === "select" ? (
                    <select
                      ...
                      aria-label={filter.label || `Filter by ${filter.key}`}
                      style={{
                        backgroundColor: "var(--input-bg)",
                        borderColor: "var(--input-border)",
                        color: "var(--input-text)",
        ...
        >
          ...
          >
            <tr>
              ...
              {config.expandable && (
                <th scope="col" aria-label="Row expand/collapse">
                  <span className="visually-hidden">Expand</span>
                </th>
              )}

              {config.columns.map((column) => {
                const isSortable = column.sortable !== false && config.sortable;
                const sortDirection =
                ...
                return (
                  <th
                    key={column.key}
                    scope="col"
                    className={`${column.headerClassName || ""} ${isSortable ? "cursor-pointer user-select-none" : ""}`}
                    style={{ width: column.width, textAlign: column.align }}
                    onClick={() => isSortable && handleSort(column.key)}
                    aria-sort={
                      isSortable ? (sortDirection ?? "none") : undefined
                    }
                  >
                    {column.header}
                    {isSortable && currentSort?.column === column.key && (
                      <i
          ...
          <tbody>
            {paginatedData.map((row, index) => {
              const rowId = config.keyExtractor(row, index);
              const isSelected =
      ...
      {config.paginated && config.pagination && (
        <div className="table-pagination-wrapper p-3">
          ...
          <nav className="table-pagination-nav" aria-label="Table pagination">
