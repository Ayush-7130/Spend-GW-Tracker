...
import { RateLimiter, isValidPassword } from "@/lib/utils/security";
import {
  parseUserAgent,
  getLocationFromIp,
  getIpAddress,
...
export async function POST(request: NextRequest) {
  try {
    // Check JWT_SECRET early to prevent obscure errors later
    ...
    // SECURITY: Store hashed token, send plaintext via email
    ...
    const userDoc = {
      ...
      // Multi-group support: New users start with no groups
      // They will be prompted to create or join a group after signup
      groups: [],
      currentGroupId: null,
    ...
    // Generate session ID (used as tokenId for revocation tracking)
    ...
    const tokenInfo = generateRefreshToken(
      {
        userId,
        email: userDoc.email,
        tokenId: sessionId.toString(), // Use session ID for token revocation tracking
      },
      rememberMe
    );
    ...
    await db.collection("sessions").insertOne({
      _id: sessionId, // Use the generated session ID
      userId,
    ...
    // Create token entry for revocation tracking
    // This allows tokens to be individually revoked if needed
    const { createTokenEntry } = await import("@/lib/auth");
    await createTokenEntry(sessionId.toString(), userId, sessionExpiresAt);
    ...
    const response = NextResponse.json(
      {
        ...
        data: {
          user: {
            _id: userId,
            id: userId, // Alias for convenience
            ...userResponse,
            groups: userDoc.groups || [],
            currentGroupId: userDoc.currentGroupId || null,
          },
          token: tokenInfo.token,
    ...
    // SECURITY: Send plaintext token via email, store hashed in DB
    ...
    logError("Signup error", error, { ip });
