...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // NOTE: currentGroupId is NOT stored in JWT - it's fetched from database on each request
    const tokenInfo = generateRefreshToken(
      {
        userId,
        email: user.email,
        tokenId: sessionId.toString(),
      },
      rememberMe
    );
    ...
    await createTokenEntry(sessionId.toString(), userId, tokenInfo.expiresAt);
    ...
    const response = NextResponse.json(
      {
        ...
        data: {
          user: {
            ...
            groups: user.groups || [],
            currentGroupId: user.currentGroupId || undefined,
          },
          session: {
