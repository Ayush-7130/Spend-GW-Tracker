...
const NotificationBell: React.FC = () => {
  ...
  const filteredNotifications = useMemo(() => {
    ...
    if (filterType === "unread") {
      ...
      // Filter by entityType (normalized field from API) first,
      // then fall back to type prefix matching
      result = result.filter(
        (n) =>
          n.entityType === filterType ||
          n.type === filterType ||
          n.type.startsWith(filterType + "_")
      );
    }
  ...
  // Show search when there are more than 10 notifications
  const showSearch = notifications.length > 10;
  ...
  const fetchNotifications = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await NotificationsDataSource.getNotifications(
        1,
        100,
        activeGroup?._id
      );
  ...
  useEffect(() => {
    if (user) {
      // Re-fetch whenever the active group changes so notifications are group-scoped
  ...
  }, [user, activeGroup?._id, fetchNotifications]);
  ...
  const getNotificationIcon = (type: string, entityType?: string) => {
    const category = entityType || type;
    if (category === "expense" || type.startsWith("expense_")) return "💰";
    if (category === "settlement" || type.startsWith("settlement_"))
      return "🤝";
    if (
      category === "group" ||
  ...
  const bellAriaExpanded: "true" | "false" = isOpen ? "true" : "false";
  ...
  return (
    <div className="position-relative" ref={bellRef}>
      <button
        ...
        aria-expanded={bellAriaExpanded}
        aria-haspopup="true"
      >
        <i
      ...
      {isOpen && (
        ...
        >
          ...
          {/* Filter and Search - Audit: NotificationBell filters */}
          {notifications.length > 0 && (
            <div
              style={{
                padding: "8px 16px",
                ...
                backgroundColor: "var(--notification-panel-header-bg)",
              }}
            >
              {/* Type Filter Buttons */}
              <div
              ...
              >
                {(
                  ...
                  const pressedValue: "true" | "false" = isActive
                    ? "true"
                    : "false";
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border: `1px solid ${isActive ? "var(--btn-primary-bg)" : "var(--notification-panel-border)"}`,
                      }}
                    >
                      {label}
                    </button>
