...
export const cacheKeys = {
  ...
  groupMembership: (userId: string, groupId: string) =>
    `group_membership:${userId}:${groupId}`,
  groupData: (groupId: string) => `group_data:${groupId}`,
  userPermissions: (userId: string, groupId: string) =>
    `user_permissions:${userId}:${groupId}`,
};
...
export const invalidateCache = {
  ...
  userGroups: (userId: string) => {
    cache.delete(cacheKeys.userGroups(userId));
    cache.deletePattern(`^group_membership:${userId}:`);
    cache.deletePattern(`^user_permissions:${userId}:`);
  },
  groupMembership: (userId: string, groupId: string) => {
    cache.delete(cacheKeys.groupMembership(userId, groupId));
    cache.delete(cacheKeys.userGroups(userId));
    cache.delete(cacheKeys.userPermissions(userId, groupId));
  },
  allGroupCaches: () => {
    cache.deletePattern("^user_groups:");
    cache.deletePattern("^group_membership:");
    cache.deletePattern("^group_data:");
    cache.deletePattern("^user_permissions:");
  },
};

/**
 * Default TTL values (in seconds, converted to milliseconds)
 */
export const CacheTTL = {
  SHORT: 60 * 1000, // 1 minute
  MEDIUM: 300 * 1000, // 5 minutes (default for group membership)
