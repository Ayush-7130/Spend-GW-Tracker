 ...
 * - Production deployment MUST override with secure random secret
 * - Missing secret in production causes immediate startup failure
 */
...
if (process.env.NODE_ENV === "production" && !process.env.JWT_SECRET) {
  throw new Error(
    "CRITICAL SECURITY ERROR: JWT_SECRET environment variable is required in production. " +
      "Generate a secure random secret (e.g., openssl rand -base64 32) and set it in your environment."
  );
}
...
export interface JWTPayload {
  userId: string;
  email: string;
  tokenId: string; // For token revocation tracking
  iat?: number;
  exp?: number;
}
 ...
 *
 * @param payload - User identification data (userId, email, tokenId)
...
export function generateRefreshToken(
  ...
  // No longer include 'type' field - simplified payload
...
export function verifyRefreshToken(token: string): JWTPayload | null {
  try {
    ...
    // FIX M59: Log token verification failures for security monitoring
...
/**
 * Create a token entry in the database for revocation tracking
 *
 * This should be called when creating a new session/token.
 * The tokenId from JWT payload is stored for revocation tracking.
 *
 * @param tokenId - The tokenId from JWT payload
...
export async function createTokenEntry(
  ...
  try {
    ...
    await db.collection("tokens").insertOne({
      tokenId, // Store as tokenId field (string), not _id
      userId,
      createdAt: new Date(),
      expiresAt,
      revoked: false,
    ...
    logger.error("[createTokenEntry] Error", error, {
      context: "createTokenEntry",
      tokenId,
      userId,
    });
    return false;
...
/**
 * Revoke a specific token by tokenId
 *
 * This allows immediate token revocation without waiting for expiry.
 * Used during logout or when a token is compromised.
 *
 * @param tokenId - The tokenId from JWT payload
 * @returns True if successful
 */
export async function revokeToken(tokenId: string): Promise<boolean> {
  try {
    ...
    const result = await db.collection("tokens").updateOne(
      { tokenId }, // Query by tokenId field
      {
        $set: {
          revoked: true,
          revokedAt: new Date(),
 ...
 *
 * This is called during getUserFromRequest to verify token is still valid.
 *
 * @param tokenId - The tokenId from JWT payload
 * @returns True if token is revoked or doesn't exist
 */
export async function isTokenRevoked(tokenId: string): Promise<boolean> {
  try {
    ...
    // If token doesn't exist or is revoked, return true
    return !token || token.revoked === true;
  } catch (error) {
    logger.error("[isTokenRevoked] Error", error, {
      context: "isTokenRevoked",
      tokenId,
    });
    // On error, fail closed (assume revoked) for security
    return true;
  }
}
...
export async function revokeAllUserTokens(userId: string): Promise<number> {
  try {
    ...
    await db.collection("securityLogs").insertOne({
      ...
      description: `Revoked all ${result.modifiedCount} active token(s)`,
      metadata: {
    ...
    logger.error("[revokeAllUserTokens] Error", error, {
      context: "revokeAllUserTokens",
      userId,
    });
    return 0;
  }
  ...
  try {
    ...
    if (!token) {
      if (process.env.NODE_ENV !== "production") {
        logger.warn("[getUserFromRequest] No token found", {
          hasAuthHeader: !!authHeader,
          hasCookie: !!request.cookies.get("refreshToken"),
        });
      }
      return null;
    }
    ...
    if (payload.tokenId) {
      const revoked = await isTokenRevoked(payload.tokenId);
      if (revoked) {
    ...
    try {
      ...
      if (!session) {
        if (process.env.NODE_ENV !== "production") {
          logger.warn("[getUserFromRequest] Session not found in database", {
      ...
      if (session.expiresAt && new Date() > new Date(session.expiresAt)) {
        if (process.env.NODE_ENV !== "production") {
          logger.warn("[getUserFromRequest] Session expired", {
      ...
      if (shouldUpdate) {
        ...
        db.collection("sessions")
          ...
          .catch((error) => {
            // Activity tracking is non-critical, but log for monitoring
            logger.error(
              "[getUserFromRequest] Failed to update lastActivityAt",
              error,
              {
      ...
      // SECURITY POSTURE: Fail-closed on DB unavailability
      // Database unavailable means we cannot validate session state (isActive, expiresAt)
      // Revoked sessions would incorrectly remain valid during DB outages
      // DECISION: Reject all requests when DB is unavailable to maintain security
      logger.error(
        "[getUserFromRequest] Database unavailable - REJECTING request for security",
        dbError,
        {
          userId: payload?.userId,
          securityNote:
            "Fail-closed posture: cannot validate session without DB access",
        }
      );
      // Return null to deny access - this maintains security during DB outages
      // Alternative: Could allow read-only routes or implement short-lived grace period
      return null;
    }
  } catch (tokenError) {
    // Token extraction or verification failed
    logger.error("[getUserFromRequest] Token verification failed", tokenError);
    return null;
  }
}
...
// Re-export isValidPassword from canonical location for backward compatibility
// All password validation uses lib/utils/security.ts as single source of truth
export {
  isValidPassword,
