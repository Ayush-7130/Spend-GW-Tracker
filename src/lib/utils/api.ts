 ...
 * Note: For new code, prefer using the datasource layer (src/datasource/*)
 * which provides centralized cache control via CacheOptions.
 */

import { CacheOptions } from "@/datasource";

/**
 * Generic data fetcher with error handling
 * Uses centralized cache settings from datasource layer
 */
export const fetchData = async <T>(
  ...
  try {
    // Use centralized cache settings from datasource
    const defaultOptions: RequestInit = {
      ...CacheOptions.noCache,
      headers: {
        ...CacheOptions.noCache.headers,
        ...(options?.headers || {}),
      },
    };
