...
/**
 * Canonical Web Vitals reporter - single source of truth for metrics reporting.
 * Used by WebVitalsReporter component and can be used directly for manual reporting.
 */
export function reportWebVitals(metric: WebVitalsMetric): void {
  if (process.env.NODE_ENV === "production") {
    ...
    // Send to custom analytics endpoint via sendBeacon for reliability
    if (typeof navigator !== "undefined" && navigator.sendBeacon) {
      ...
      // sendBeacon has 64KB limit - vitals data is always well under this
