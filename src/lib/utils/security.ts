...
export interface PasswordValidationResult {
  valid: boolean;
  errors: string[];
  /** Single error message (first error) for backward compatibility */
  message?: string;
}

/**
 * Password validation requirements.
 * Centralized constants for consistent password policy across the application.
 */
...
/**
 * CANONICAL: Validate password against security requirements
 *
 * Single source of truth for password validation across the application.
 * All password validation should import and use this function.
 *
 * Requirements:
 * - At least 8 characters (configurable via PASSWORD_REQUIREMENTS)
 * - At least 1 uppercase letter
 * - At least 1 lowercase letter
 * - At least 1 number
 * - At least 1 special character (!@#$%^&*(),.?":{}|<>)
 *
 * WHY Require Special Characters:
 * - Significantly increases password entropy
 * - Consistent with industry standards (OWASP, NIST)
 * - auth-edge.ts and reset-password already required them
 *
 * WHY Return Errors Array:
 * - Better UX: Show all issues at once, not one-by-one
 * - Helps users create compliant passwords faster
 * - message field provided for backward compatibility
 */
export function isValidPassword(password: string): PasswordValidationResult {
  const errors: string[] = [];
  const { minLength, specialCharPattern } = PASSWORD_REQUIREMENTS;
  ...
  return {
    ...
    errors,
    message: errors[0], // First error for backward compatibility
  };
}

/**
 * Simple boolean password validation.
 * Wraps isValidPassword for code that just needs a boolean check.
 */
 ...
 *
 * Uses crypto.getRandomValues() for browser or crypto.randomBytes() for Node.js.
 *
 * WHY Crypto APIs:
 * - Math.random() is NOT cryptographically secure (predictable)
 * - crypto APIs use OS-level entropy (truly random)
