...
export function exportExpensesToCSV(expenses: ExpenseExportData[]): string {
  // Build dynamic columns based on unique users in the dataset
  const allUsers = new Set<string>();
  expenses.forEach((expense) => {
    if (expense.splitDetails?.splits) {
      expense.splitDetails.splits.forEach((split) => {
        allUsers.add(split.userName);
      });
    }
  });
  ...
  const baseColumns = [
    {
      key: "date",
      label: "Date",
      format: (value: any) => formatDate(value, { dateStyle: "short" }),
    },
    { key: "description", label: "Description" },
    {
      key: "categoryName",
      label: "Category",
      format: (value: any, row: any) => value || row.category,
    },
    {
      key: "amount",
      label: "Amount (INR)",
    ...
    },
    {
      key: "paidBy",
      label: "Paid By",
      format: (value: any) => value.charAt(0).toUpperCase() + value.slice(1),
    },
    {
      key: "isSplit",
      label: "Type",
      format: (value: any) => (value ? "Split" : "Personal"),
    },
  ];

  // Add dynamic user split columns
  const userColumns = Array.from(allUsers)
    .sort()
    .map((userName) => ({
      key: "splitDetails",
      label: `${userName} Amount (INR)`,
      format: (value: any, row: any) => {
        if (!row.isSplit || !value?.splits) return "";
        const userSplit = value.splits.find(
          (s: any) => s.userName === userName
        );
        return userSplit ? userSplit.amount.toString() : "";
      },
    }));

  return arrayToCSV(expenses, [...baseColumns, ...userColumns] as any);
}
